...
describe('Robots API - validation', () => {
    ...
    const statuses = ['IDLE', 'ASSIGNED', 'EN_ROUTE', 'CHARGING', 'MAINTENANCE', 'OFFLINE'];
    test.each(statuses.map((s) => [s]))('creates robot with status %p and updates battery', async (s) => {
        ...
        const patch = await (0, supertest_1.default)(app).patch(`/api/robots/${id}`).send({ batteryPercent: 75 });
...
//# sourceMappingURL=robots.validation.test.js.map
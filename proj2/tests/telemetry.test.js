...
describe('Telemetry API', () => {
    it('returns a snapshot of 5 simulated robots', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/telemetry/snapshot');
    ...
    it('exposes an SSE stream endpoint with correct headers', async () => {
        ...
        const snapshot = await (0, supertest_1.default)(app).get('/api/telemetry/snapshot');
    ...
    it('accepts a simulated stop command', async () => {
        ...
        const stop = await (0, supertest_1.default)(app).post(`/api/telemetry/robots/${id}/stop`);
        expect(stop.status).toBe(202);
        // Verify robot is now OFFLINE in a new snapshot
        const after = await (0, supertest_1.default)(app).get('/api/telemetry/snapshot');
        const stopped = after.body.find((r) => r.id === id);

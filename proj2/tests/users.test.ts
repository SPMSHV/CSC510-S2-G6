...
describe('Users API', () => {
  ...
  describe('Error Paths', () => {
    ...
    it('rejects update with invalid email format', async () => {
      ...
      const res = await request(app).patch(`/api/users/${id}`).send({ email: 'invalid-email' });
      expect(res.status).toBe(400);
    });
    ...
    it('rejects update with empty payload', async () => {
      ...
      const res = await request(app).patch(`/api/users/${id}`).send({});
      expect(res.status).toBe(400);
    });

    it('handles invalid UUID format in get request', async () => {
      const res = await request(app).get('/api/users/invalid-uuid-format');
      expect(res.status).toBe(404);
    });

    it('handles invalid UUID format in patch request', async () => {
      const res = await request(app).patch('/api/users/invalid-uuid').send({ name: 'Test' });
      expect(res.status).toBe(404);
    });

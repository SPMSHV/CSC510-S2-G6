...
describe('Restaurants API', () => {
    ...
    beforeEach(async () => {
        ...
        const studentReg = await (0, supertest_1.default)(app).post('/api/auth/register').send({
        ...
        const adminReg = await (0, supertest_1.default)(app).post('/api/auth/register').send({
        ...
        const restaurantRes = await (0, supertest_1.default)(app)
        ...
        if (restaurantRes.status === 201 && restaurantRes.body.id) {
            ...
            const allRestaurants = await (0, supertest_1.default)(app).get('/api/restaurants');
            const found = allRestaurants.body.find((r) => r.vendorId === vendorId);
            if (found) {
    ...
    describe('GET /api/restaurants', () => {
        it('returns empty array initially before any restaurants are created', async () => {
            ...
            const freshApp = (0, request_1.createApp)();
            const res = await (0, supertest_1.default)(freshApp).get('/api/restaurants');
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
        });
        it('returns all restaurants', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/restaurants');
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
    ...
    describe('GET /api/restaurants/:id', () => {
        it('successfully gets restaurant by ID', async () => {
            ...
            const res = await (0, supertest_1.default)(app).get(`/api/restaurants/${restaurantId}`);
            if (restaurantId !== 'test-restaurant-id-placeholder') {
                expect(res.status).toBe(200);
        ...
        it('returns 404 for non-existent restaurant', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/restaurants/nonexistent-id');
            expect(res.status).toBe(404);
    ...
    describe('GET /api/restaurants/:id/menu', () => {
        it('returns empty menu initially', async () => {
            const res = await (0, supertest_1.default)(app).get(`/api/restaurants/${restaurantId}/menu`);
            expect(res.status).toBe(200);
            expect(Array.isArray(res.body)).toBe(true);
    ...
    describe('POST /api/restaurants', () => {
        it('successfully creates restaurant with VENDOR auth', async () => {
            ...
            const newVendorReg = await (0, supertest_1.default)(app).post('/api/auth/register').send({
            ...
            const res = await (0, supertest_1.default)(app).post('/api/restaurants').set('Authorization', `Bearer ${newVendorToken}`).send(payload);
        ...
        it('rejects without authentication', async () => {
            ...
            const res = await (0, supertest_1.default)(app).post('/api/restaurants').send(payload);
        ...
        it('rejects non-VENDOR users', async () => {
            ...
            const res = await (0, supertest_1.default)(app).post('/api/restaurants').set('Authorization', `Bearer ${studentToken}`).send(payload);
        ...
        it('rejects duplicate restaurant for same vendor', async () => {
            ...
            const testVendorReg = await (0, supertest_1.default)(app).post('/api/auth/register').send({
            ...
            const res1 = await (0, supertest_1.default)(app).post('/api/restaurants').set('Authorization', `Bearer ${testVendorToken}`).send(payload);
            ...
            const res2 = await (0, supertest_1.default)(app).post('/api/restaurants').set('Authorization', `Bearer ${testVendorToken}`).send({
        ...
        it('validates required fields', async () => {
            const res = await (0, supertest_1.default)(app).post('/api/restaurants').set('Authorization', `Bearer ${vendorToken}`).send({});
            expect(res.status).toBe(400);
        });
    });
    describe('PATCH /api/restaurants/:id', () => {
        ...
        it('rejects unauthorized users', async () => {
            ...
            const otherVendor = await (0, supertest_1.default)(app).post('/api/auth/register').send({
        ...
        it('rejects without authentication', async () => {
            const res = await (0, supertest_1.default)(app).patch(`/api/restaurants/${restaurantId}`).send({ name: 'Unauthorized' });
            expect(res.status).toBe(401);
        });
    ...
    describe('POST /api/restaurants/:id/menu', () => {
        ...
        it('rejects without authentication', async () => {
            const res = await (0, supertest_1.default)(app).post(`/api/restaurants/${restaurantId}/menu`).send({ name: 'Item', price: 10 });
            expect(res.status).toBe(401);
        });
        it('validates required fields', async () => {
            const res = await (0, supertest_1.default)(app)
    ...
    describe('Error Paths', () => {
        ...
        it('handles invalid UUID format in get request', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/restaurants/invalid-uuid');
            expect(res.status).toBe(404);
        });
        it('handles invalid UUID format in menu get request', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/restaurants/invalid-uuid/menu');

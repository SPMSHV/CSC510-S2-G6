...
describe('Orders API', () => {
  ...
  describe('Error Paths', () => {
    ...
    it('handles invalid UUID format in get request', async () => {
      const res = await request(app).get('/api/orders/invalid-uuid-format');
      expect(res.status).toBe(404);
    });
    ...
    it('handles invalid UUID format in delete request', async () => {
      const res = await request(app).delete('/api/orders/invalid-uuid');
      expect(res.status).toBe(404);
    });
  ...
  describe('Order Tracking Endpoints', () => {
    ...
    describe('GET /api/orders/my-orders', () => {
      it('returns orders for authenticated user', async () => {
        const res = await request(app).get('/api/orders/my-orders').set('Authorization', `Bearer ${userToken}`);
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body)).toBe(true);
      ...
      it('returns empty array if user has no orders', async () => {
        const newUserReg = await request(app).post('/api/auth/register').send({
        ...
        const res = await request(app).get('/api/orders/my-orders').set('Authorization', `Bearer ${newUserToken}`);
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body)).toBe(true);
        expect(res.body.length).toBe(0);
      });
      ...
      it('filters by user ID correctly', async () => {
        ...
        const res = await request(app).get('/api/orders/my-orders').set('Authorization', `Bearer ${userToken}`);
        expect(res.status).toBe(200);
        const allOrdersBelongToUser = res.body.every((o: { userId: string }) => o.userId === userId);
    ...
    describe('GET /api/orders/:id/track', () => {
      it('returns full tracking info with progress', async () => {
        const res = await request(app).get(`/api/orders/${orderId}/track`).set('Authorization', `Bearer ${userToken}`);
        expect(res.status).toBe(200);
      ...
      it('returns 403 for unauthorized access', async () => {
        ...
        const res = await request(app).get(`/api/orders/${orderId}/track`).set('Authorization', `Bearer ${otherUserToken}`);
        expect(res.status).toBe(403);
      });

      it('returns 404 for non-existent order', async () => {
        const res = await request(app).get('/api/orders/nonexistent-id/track').set('Authorization', `Bearer ${userToken}`);
        expect(res.status).toBe(404);
      });
    ...
    describe('GET /api/orders/:id/status', () => {
      ...
      it('returns 403 for unauthorized access', async () => {
        ...
        const res = await request(app).get(`/api/orders/${orderId}/status`).set('Authorization', `Bearer ${otherUserToken}`);
        expect(res.status).toBe(403);
      });
    ...
    describe('Robot Assignment Logic', () => {
      it('assigns nearest robot when order status changes to READY', async () => {
        ...
        const robot1Res = await request(app).post('/api/robots').send({
          robotId: 'RB-NEAR',
          status: 'IDLE',
          batteryPercent: 90,
        ...
        const robot2Res = await request(app).post('/api/robots').send({
          robotId: 'RB-FAR',
          status: 'IDLE',
          batteryPercent: 85,
      ...
      it('handles case when no robots available', async () => {
        ...
        const unavailableRobot = await request(app).post('/api/robots').send({
          robotId: 'RB-BUSY',
          status: 'ASSIGNED',
          batteryPercent: 90,
          location: { lat: 35.0, lng: -78.0 },
